...
/*   Updated: 2025/05/27 11:20:25 by ljudd            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ponderate_mean(int color1, int color2, double w)
{
	...
	rgb1[0] = (color2) & 0xff;
	rgb1[1] = (color2 >> 8) & 0xff;
	rgb1[2] = (color2 >> 16) & 0xff;
...
void	set_color(t_fractol *f)
{
	if (!f->mlx)
		return ;
	f->palette = malloc(N_ITER * sizeof(int));
	if (!f->palette)
		return ;
	set_color_neon(f);
}
